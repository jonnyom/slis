...
- **Every read command emits `--json`:** `ls show status pr pr-stack summary
  conflicts comments doctor candidates branch-diff tree cat`. Prefer it over
  parsing tables.
...
stale-journal deletion under `--fix`; doctor never prunes or deletes worktrees.
...
|---|---|---|
| **read-only** | `ls show status pr pr-stack summary conflicts comments doctor candidates branch-diff tree cat edit` | Safe anytime. `doctor --fix` is the exception (it mutates). |
