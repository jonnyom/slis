...
| Concept | Definition |
|---|---|
| **Project root** | The directory the repos live under (e.g. `~/nory`). `slis init` scans it, shows a picker, and you choose which repos every slice spans. |
| **Repo** | A **selected** codebase under the root. Each has a fixed **primary tree** (where dev servers run, layout "A") plus N worktrees. The selected set is "always included" — `create` materialises a worktree in *every* selected repo. Unselected repos under the root (e.g. `infra`) are ignored. |
...
```yaml
# ~/.config/slis/workspace.yaml  (generated by `slis init`, hand-editable)
root: ~/nory                     # project root scanned for repos
repos:                           # the SELECTED set — every slice spans these ("always duplicated")
  nory:            { primary: ~/nory/nory,            default_branch: main }
  node-middleware: { primary: ~/nory/node-middleware, default_branch: main }
  web-app:         { primary: ~/nory/web-app,         default_branch: main }
  # ~/nory/infra was discovered but left unselected → absent here
...
## 6. Repo Selection & Slice Discovery

### Repo selection — `slis init` (one-time, re-runnable)
1. Scan the **project root** (default = cwd, or `slis init <root>`) for git repos among its direct children (a dir containing `.git`).
2. Present a **multi-select picker** of the discovered repos.
3. You tick the repos every slice should span (`nory`, `node-middleware`, `web-app`); leave others (`infra`) unticked.
4. Write `root` + the selected repos (primary path + detected default branch) to `workspace.yaml`. Re-running `init` reconciles the selection against the current scan.

The selected set is **canonical**: `slis create <slice>` materialises a worktree in **every** selected repo ("always duplicated"). A repo with no matching worktree simply appears as an empty slot in a discovered slice (still a member, on mainline), never silently dropped.

### Slice discovery
On launch (and on fsnotify change), for each **selected** repo:
...
```
slis init [<root>]            # scan root for repos, multi-select picker → workspace.yaml
