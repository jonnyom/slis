...
- Homebrew install is a **prebuilt cask** in `jonnyom/homebrew-tap` (`brew install jonnyom/homebrew-tap/slis`), published by GoReleaser (the `homebrew_casks` block in `.goreleaser.yaml`) on every `vX.Y.Z` tag. The release workflow needs a `HOMEBREW_TAP_GITHUB_TOKEN` repo secret (PAT with write access to the tap) to push the cask. Both shipped binaries are unsigned, so the cask's `postflight` strips the macOS quarantine flag off **both** `slis` and `slis-ui`. (Cut a release by tagging: `git tag vX.Y.Z && git push origin vX.Y.Z`.)
- **The JS TUI (`slis-ui`) ships alongside `slis`.** Each release archive contains `slis` *and* its platform-matched `slis-ui` at the archive root, so an installed `slis` finds its sibling (`internal/cli.resolveUILaunch` prefers a compiled `slis-ui` next to the running binary) and bare `slis` launches the JS front-end. `slis-ui` is built by a GoReleaser `before` hook (`scripts/build-slis-ui.sh`): `bun install --frozen-lockfile --cpu '*' --os '*'` pulls every platform's `@opentui/core-<plat>` optional dep, then `bun build --compile --target=bun-{darwin,linux}-{x64,arm64}` cross-compiles one `slis-ui` per target into `tui-js/dist/<goos>-<goarch>/slis-ui`. The `archives.files` entry picks each with `{{ .Os }}`/`{{ .Arch }}` + `strip_parent`. Bun's compile embeds ONE platform's native libs (OpenTUI Zig core + ghostty VT addon), so cross-compile only works with the target's optional deps installed — hence the `--cpu '*' --os '*'`. The release workflow installs `bun` via `oven-sh/setup-bun` before GoReleaser runs. Linux `slis-ui` is glibc-linked (dynamic), unlike the fully-static `CGO_ENABLED=0` Go `slis` — musl-only distros (Alpine) won't run it; the Go CLI still works there.
