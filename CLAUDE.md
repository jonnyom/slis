...
| `restack` | run `gt restack` across a slice's repos (refuses dirty worktrees; conflicts left for the user). `slis restack` / `slis submit` (stack→PRs) / `slis sync` (TUI `R` → stack-actions overlay) |
...
**Integrations.** `forge` is read-only (`gh pr view` only); `forge.PRForBranch` returns `(nil,nil)` when there's no PR or `gh` is absent — callers tolerate per-repo failures. `gt` is read-only *except* `gt.Restack` (the one mutator), always run behind a TUI confirm / explicit `slis restack`; the `internal/restack` engine refuses dirty worktrees and never auto-stashes/aborts (conflicts are left for the user to resolve + `gt continue`). `slis sync` (interactive `gt sync` per repo; repo-wide — may overwrite trunk, delete merged branches) and `slis submit` (interactive `gt submit` per repo — force-pushes the stack + opens/updates PRs) shell out to `gt` directly (CLI-level, not via the `gt` package).
