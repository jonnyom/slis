...
export function Cockpit(props: CockpitProps): ReactNode {
  ...
  const footer = useMemo(() => {
    ...
    switch (panel) {
      case "stack":
        return `tab panel · j/k repo · b scope:${SCOPE_LABEL[scope]} · t ${showPatch ? "stat" : "patch"} · ${common}`;
      case "prs":
        return `tab panel · j/k pr · O open PR · ${common}`;
      case "session":
        return `tab panel · ${common}`;
      case "procs":
        return `tab panel · j/k proc · ${common}`;
    }
